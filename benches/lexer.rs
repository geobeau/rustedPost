...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("homemade (heap) mode", |b| b.iter(|| lexer::parse_record(black_box(field_custom))));
    c.bench_function("custom (stack) mode", |b| b.iter(|| lexer::parse_small_record(black_box(field_custom))));
    c.bench_function("custom (logos) mode", |b| b.iter(|| lexer::parse_record_logos(black_box(field_custom))));
}
