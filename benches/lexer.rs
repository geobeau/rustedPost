...
use rusted_post::lexer::parse_record;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("homemade (heap) mode", |b| b.iter(|| parse_record(black_box(field_custom))));
    c.bench_function("custom (stack) mode", |b| b.iter(|| parse_record(black_box(field_custom))));
}
