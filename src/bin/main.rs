...
async fn main() {
    ...
    let level = match matches.value_of("log-level").unwrap() {
        ...
        _ => {println!("Log level didn't match a known level (debug, info, err, warn)"); return;}
    };
    ...
    let threads = match matches.value_of("threads").unwrap().parse() {
        Ok(x) => x,
        Err(err) => {error!("Error while getting thread count: {}", err); return}
    };

    let storage = Arc::new(RwLock::new(
        backend::ShardedStorageBackend::new_with_cpus(threads)));
